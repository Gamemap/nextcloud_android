	...
	private function createFinalTemplate(): IEMailTemplate {
		...
		if ($scheduled) {
			if ($purgeDays === 0) {
				...
				$template->addBodyText($this->l10n->n('Your data will be permanently deleted in %d day, on %s.', 'Your data will be permanently deleted in %d days, on %s.', $purgeDays, [$purgeDays, $formattedDate]));
			}
